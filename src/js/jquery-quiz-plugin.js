(function( $ ) {
	$.fn.quizify = function( options ) {
		...
		return this.each( function(customText) {
			$.each($(this).children("input:text"),function(i,item){
				var p = $(item).attr("answ");
				$(item).attr("size",p.length);
				var ins_help = settings.help;
				if(typeof $(item).attr("help") != 'undefined') {
					var a = $(item).attr("help");
					ins_help = ($(item).attr("help") == 'true');
				} 
